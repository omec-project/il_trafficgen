...
// Pktgen-tool: Header for pktgen-tool utilities
...
{
        FUNC_TRACE("P%d: $protocol = %s", info->pid, type);
...
{
        FUNC_TRACE("P%d: $pkt_type = %s", info->pid, type);
...
{
        FUNC_TRACE("P%d: $count = %u", info->pid, cnt);
...
/**************************************************************************//**
 *
 * pktgen_set_port_pps - Set the PPS for a port
 *
 * DESCRIPTION
 * Set a packets per second for all ports in the list.
 *
 * RETURNS: N/A
 *
 * SEE ALSO:
...
void
pktgen_set_port_pps(port_info_t *info, uint64_t pps)
{
        FUNC_TRACE("P%d: $pps = %lu", info->pid, pps);
      
	info->tx_pps = pps;
	uint64_t wire_size = (pktgen_wire_size(info) * 8);    //  wire_size = info->seq_pkt[SINGLE_PKT].pktSize +
				                              //  PKT_PREAMBLE_SIZE + INTER_FRAME_GAP + FCS_SIZE; = (1020+8+12+4)*8=1044*8=8352
	uint64_t link = (uint64_t)info->link.link_speed * Million; // Million: 1000000     // link = 10Gb
        info->tx_rate = pps * 100 *((double)wire_size/(double)link); // 500,000*100*(8352/10Gb) = 41.76
        FUNC_TRACE("P%d: tx_rate is %f", info->pid, info->tx_rate);
...
{
        FUNC_TRACE("P%d: $burst = %u", info->pid, burst);
	if (burst == 0)
		burst = 1;
	else if (burst > DEFAULT_PKT_BURST)
		burst = DEFAULT_PKT_BURST;
	info->tx_burst = burst;
        // pktgen-tool: Commented because PPS is manually set
        // pktgen_packet_rate(info);
}
...
{
        FUNC_TRACE("P%d: $pkt_size = %u", info->pid, size);
	pkt_seq_t* pkt = &info->seq_pkt[SINGLE_PKT];
...
single_set_tx_rate(port_info_t *info, const char *r)
{
	...
	pktgen_packet_rate(info);
        FUNC_TRACE("PPS for tx_rate of 41.76 is %lu", info->tx_pps);
}
...
{
        char ip_addr[32] = {0};
        inet_ntop(AF_INET, &ip->ipv4, ip_addr, 32);
	if (type == 's') {
                FUNC_TRACE("P%d: $src_ip = %s/%u", info->pid, ip_addr, ip->prefixlen);
                if (traffic_gen_as == IL_TRAFFIC_GEN)
                  start_ip =  ntohl(ip->ipv4.s_addr); 
		info->seq_pkt[SINGLE_PKT].ip_mask = size_to_mask(ip->prefixlen);
		info->seq_pkt[SINGLE_PKT].ip_src_addr.addr.ipv4.s_addr = ntohl(ip->ipv4.s_addr); 
	} else {
                FUNC_TRACE("P%d: $dst_ip = %s", info->pid, ip_addr);
...
{
        FUNC_TRACE("P%d: $dst_mac = %02x:%02x:%02x:%02x:%02x:%02x", info->pid, mac->addr_bytes[0], 
...
single_set_src_mac(port_info_t *info, struct ether_addr *mac)
{
        ...
        FUNC_TRACE("P%d: $src_mac = %02x:%02x:%02x:%02x:%02x:%02x", info->pid, mac->addr_bytes[0], 
                   mac->addr_bytes[1], mac->addr_bytes[2], mac->addr_bytes[3],
                   mac->addr_bytes[4], mac->addr_bytes[5]);
...
range_set_src_ip(port_info_t *info, char *what, struct pg_ipaddr *ip)
{
	if (!strcmp(what, "min") || !strcmp(what, "minimum")) {
		info->range.src_ip_min = ntohl(ip->ipv4.s_addr);
                ...
                FUNC_TRACE("P%d: $subs = %lu", info->pid, subs);
