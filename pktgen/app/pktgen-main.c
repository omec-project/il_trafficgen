...
#endif
#ifdef PCAP_GEN
char PORT0_PCAP_FILE[256] = "./autotest/log/port_gen.pcap";
char PORT1_PCAP_FILE[256] = "./autotest/log/port_resp.pcap";
#endif /* PCAP_GEN */
/**************************************************************************//**
 *
...
pktgen_parse_args(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argvopt, "p:m:f:l:s:g:hPNGTtrx",
		...
		case 0:	/* crc-strip for all ports */
			...
			break;
		case 't': /* run il_trafficgen as generator */
			traffic_gen_as = IL_TRAFFIC_GEN;
			break;
		case 'r': /* run il_trafficgen as responder */
			traffic_gen_as = IL_TRAFFIC_RESP;
			break;
		case 'x': /* run il_trafficgen as responder_as_reflector */
...
main(int argc, char **argv)
{
	...
	/* Parse User configuration from ../autotest/user_input.cfg */
	parse_user_config();
    ...
    FUNC_TRACE("Start Pktgen-testpmd application");
	/* register sockets for communication b/w generator and responder */
	...
	if (traffic_gen_as == IL_TRAFFIC_GEN) {
		printf("\n\n GENERATOR pcap file will be overwritten."
			" Press ENTER to continue...\n");
		getchar();
		pcap_dumper_p0 = init_pcap(PORT0_PCAP_FILE);
		if (pcap_dumper_p0 == NULL) {
			printf("Error in opening pcap file!!!\n");
			return -1;
		}
	} else {
		printf("\n\n REPSONDER pcap file will be overwritten."
			" Press ENTER to continue...\n");
		getchar();
		pcap_dumper_p1 = init_pcap(PORT1_PCAP_FILE);
		if (pcap_dumper_p1 == NULL) {
			printf("Error in opening pcap file!!!\n");
			return -1;
		}
	}
	...
	/* Spawn thread on display core to send pkt cnts to generator */
	if (traffic_gen_as == IL_TRAFFIC_RESP_AS_REF ||
		traffic_gen_as == IL_TRAFFIC_RESP) {
		pthread_t t;
		if (pthread_create(&t, NULL, send_stats_data, NULL) != 0) {
